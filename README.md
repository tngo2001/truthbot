# TruFraudBot — Gemini chatbot (C# / .NET)
...
3. **Create a `.env` file** in the project folder with:
   ...
   ```
4. **Console mode:** Run `dotnet run`. If `DISCORD_TOKEN` is not set, you get the local chatbot (type to chat, `/clear`, `/quit`).
5. **Discord mode:** Add `DISCORD_TOKEN` to `.env`, [create a bot and invite it](#discord-get-token-set-up-and-test), then run `dotnet run`. In Discord: `tb hello` (normal chat), `fb hello` (chat that follows your rules), `fb addrule Be concise`, `fb listrules`.
...
- **.NET 8 SDK** — required to build and run. If `dotnet` is not found:
  - **macOS (Homebrew):** `brew install dotnet@8` then run `brew link --overwrite dotnet@8` (or add to PATH).
...
Remove or comment out `DISCORD_TOKEN` in `.env`, run `dotnet run` — the app runs as a local console chatbot (type to chat, `/clear`, `/quit`).
...
- `Services/GeminiService.cs` — Gemini client: `ChatAsync()` (with history), `ChatOnceAsync()` (one-shot for fb).
- `Services/DiscordBotService.cs` — Discord bot: `tb` and `fb` prefixes, rules commands.
- `Services/RulesService.cs` — Read/write `rules.txt` for fb rules.
